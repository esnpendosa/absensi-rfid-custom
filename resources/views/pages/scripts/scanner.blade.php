<script>
    ...
    let popupQrInstance = null;
    let popupCameraList = [];
    let popupCameraIndex = 0;
    let popupLastScanAt = 0;
    let beepAudioContext = null;
    let beepUnlockBound = false;
    ...
    let usbScannerLastKeyAt = 0;
    const USB_SCANNER_KEY_GAP_MS = 50;
    const USB_SCANNER_MIN_LENGTH = 4;
    let siswaCardUsbWedgeBound = false;
    let siswaCardUsbBuffer = '';
    let siswaCardUsbLastKeyAt = 0;
    const SISWA_CARD_USB_KEY_GAP_MS = 50;
    const SISWA_CARD_USB_MIN_LENGTH = 4;
    const SCAN_MODE_QR = 'qr';
    const SCAN_MODE_RFID = 'rfid';
    let activeScanMode = SCAN_MODE_QR;
...
function renderScannerScopeInfo() {
    const infoEl = document.getElementById('scannerScopeInfo');
    if (!infoEl) return;

    const currentRole = String(currentUser?.role || role || '').toLowerCase();
    ...
    if (kelas !== '') {
        infoEl.className = 'rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800';
        infoEl.innerHTML = `<i class="fas fa-lock mr-1.5"></i>Mode Scan: kelas <span class="font-extrabold">${kelas}</span>`;
    } else {
        infoEl.className = 'rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800';
        infoEl.innerHTML = '<i class="fas fa-globe mr-1.5"></i>Mode Scan: semua kelas';
    }
    ...
    function isRfidMode() {
        return activeScanMode === SCAN_MODE_RFID;
    }

    function buildScanEmptyRowHtml() {
        const iconClass = isRfidMode() ? 'fa-id-card' : 'fa-qrcode';
        const hintText = isRfidMode()
            ? 'Scan RFID USB untuk menambahkan siswa'
            : 'Scan QR untuk menambahkan siswa';

        return `<tr id="scan-empty-row"><td colspan="6" class="py-16 text-center">
            <div class="flex flex-col items-center gap-3">
                <i class="fas ${iconClass} text-5xl text-gray-200"></i>
                <p class="font-semibold text-sm text-gray-400">Belum ada siswa dalam antrian</p>
                <p class="text-xs text-gray-300">${hintText}</p>
            </div></td></tr>`;
    }

    function applyScannerModeUI() {
        const qrBtn = document.getElementById('scanModeQrBtn');
        const rfidBtn = document.getElementById('scanModeRfidBtn');
        const headerTitle = document.getElementById('scanModeHeaderTitle');
        const headerSubtitle = document.getElementById('scanModeHeaderSubtitle');
        const headerIcon = document.getElementById('scanModeHeaderIcon');
        const modeHint = document.getElementById('scanModeHint');
        const qrControls = document.getElementById('qrControls');
        const manualLabel = document.getElementById('manualScanLabel');
        const manualHint = document.getElementById('manualScanHint');
        const manualInput = getManualNisnInput();
        const useRfid = isRfidMode();

        if (qrBtn) {
            qrBtn.className = useRfid
        ...
        if (rfidBtn) {
            rfidBtn.className = useRfid
                ? 'px-3 py-2 rounded-xl text-xs font-bold border border-indigo-200 bg-indigo-600 text-white shadow-sm'
                : 'px-3 py-2 rounded-xl text-xs font-bold border border-gray-200 bg-gray-100 text-gray-600';
        }

        if (headerTitle) {
            headerTitle.textContent = useRfid ? 'Scan RFID USB Absensi' : 'Scan QR Absensi';
        }
        if (headerSubtitle) {
            headerSubtitle.textContent = useRfid
                ? 'Gunakan RFID reader USB (keyboard-wedge), lalu scan kartu.'
                : 'Setiap scan langsung divalidasi & disimpan';
        }
        if (headerIcon) {
            headerIcon.innerHTML = useRfid
                ? '<i class="fas fa-id-card text-xl"></i>'
                : '<i class="fas fa-qrcode text-xl"></i>';
        }
        if (modeHint) {
            modeHint.textContent = useRfid
                ? 'Mode RFID aktif: kamera dan upload foto QR dinonaktifkan.'
                : 'Mode QR aktif: bisa pakai kamera, foto QR, atau scanner USB keyboard-wedge.';
        }
        if (qrControls) {
            qrControls.classList.toggle('hidden', useRfid);
        }
        if (manualLabel) {
            manualLabel.textContent = useRfid ? 'Input UID RFID / Scanner USB' : 'Input NISN / Scanner USB';
        }
        if (manualHint) {
            manualHint.textContent = useRfid
                ? 'Mode USB keyboard-wedge: scan UID RFID lalu Enter otomatis.'
                : 'Mode USB keyboard-wedge: scan kode lalu Enter.';
        }
        if (manualInput) {
            manualInput.placeholder = useRfid ? 'Scan/Ketik UID RFID...' : 'Scan/Ketik kode siswa...';
            manualInput.setAttribute('inputmode', useRfid ? 'text' : 'numeric');
        }

        const tbody = document.getElementById('tbody-scan-live');
        if (tbody && !tbody.querySelector('tr[data-nisn]')) {
            tbody.innerHTML = buildScanEmptyRowHtml();
        }
    }

    function setScannerMode(mode) {
        const normalizedMode = String(mode || '').toLowerCase() === SCAN_MODE_RFID
            ? SCAN_MODE_RFID
            : SCAN_MODE_QR;

        activeScanMode = normalizedMode;
        usbScannerBuffer = '';

        if (isRfidMode()) {
            stopPolling();
            if (cameraPopup && !cameraPopup.closed) {
                try { cameraPopup.close(); } catch (e) {}
            }
            cameraPopup = null;
        }

        applyScannerModeUI();
        setTimeout(() => focusManualNisnInput(), 80);
    }
...
function saveSiswa(e, isEdit) {
    ...
    const siswaData = {
        ...
        noHp: fd.get('noHp'),
        kelas: fd.get('kelas'),
        nomorKartu: String(fd.get('nomorKartu') ?? '').trim().toUpperCase(),
...
function loadQRCodeSiswa() {
    ...
    function hasSecureCameraContext() {
        ...
        return window.isSecureContext || isLocal || window.location.protocol === 'https:';
    }

    function formatCameraErrorMessage(err) {
        const name = String(err?.name || '');
        const message = String(err?.message || '');
        const raw = `${name} ${message} ${String(err || '')}`.trim();

        if (/NotAllowedError|Permission denied|Permission dismissed|denied/i.test(raw)) {
            return 'Izin kamera ditolak. Klik ikon kamera di address bar lalu pilih Allow, kemudian refresh halaman.';
        }
        if (/NotFoundError|DevicesNotFoundError|Requested device not found/i.test(raw)) {
            return 'Tidak ada perangkat kamera yang terdeteksi.';
        }
        ...
        if (/SecurityError|Only secure origins|secure context/i.test(raw)) {
            return 'Kamera hanya bisa dipakai pada HTTPS atau localhost.';
        }
        if (/AbortError/i.test(raw)) {
            return 'Akses kamera dibatalkan oleh browser/perangkat.';
        }
        if (/OverconstrainedError|ConstraintNotSatisfiedError/i.test(raw)) {
            return 'Konfigurasi kamera tidak cocok dengan perangkat.';
        }
        if (/getUserMedia is not implemented|mediaDevices/i.test(raw)) {
            return 'Browser tidak mendukung akses kamera. Gunakan Chrome/Edge versi terbaru.';
        }

        return `Gagal mengakses kamera: ${message || raw || 'Unknown error'}`;
    }

    function getBeepAudioContext() {
        if (beepAudioContext) return beepAudioContext;
    ...
    function bindBeepUnlockEvents() {
        if (beepUnlockBound) return;
        beepUnlockBound = true;
    ...
    function setPopupLoadingScreen(popupWindow) {
        if (!popupWindow || popupWindow.closed) return;
        try {
            popupWindow.document.title = 'Meminta Izin Kamera';
            popupWindow.document.body.innerHTML = `
                <div style="margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#0f172a;color:#e2e8f0;font-family:Inter,sans-serif;padding:24px;box-sizing:border-box;">
                    <div style="max-width:420px;width:100%;background:#1e293b;border:1px solid #334155;border-radius:16px;padding:20px;text-align:center;">
                        <div style="font-size:14px;font-weight:700;margin-bottom:8px;">Meminta Akses Kamera...</div>
                        <div style="font-size:12px;opacity:.85;">Jika browser menampilkan popup izin, pilih <b>Allow</b>.</div>
                    </div>
                </div>
            `;
        } catch (e) {}
    ...
    function openCameraPopup() {
        ...
        if (activeScanMode !== SCAN_MODE_QR) {
            showAlert('error', 'Mode RFID aktif. Ubah ke mode QR Code untuk membuka kamera.');
            return;
        }
        ...
        if (!hasSecureCameraContext()) {
            Swal.fire({
                icon: 'error',
                title: 'Izin Kamera Ditolak Browser',
                html: 'Kamera hanya bisa dipakai di <b>HTTPS</b> atau <b>localhost</b>.<br>Gunakan URL HTTPS / localhost lalu coba lagi.',
        ...
        if (cameraPopup && !cameraPopup.closed) {
            try {
                cameraPopup.focus();
                startPolling();
                syncQueueToCameraTab('SCAN_QUEUE_SYNC', { items: collectQueueSnapshot() });
            } catch (e) {}
            return;
        }

        const popupUrl = `${window.APP_ROUTES?.scanner || '/scanner'}?camera=1`;
        cameraPopup = window.open('about:blank', '_blank');
        if (!cameraPopup) {
            showAlert('error', 'Tab kamera diblokir browser. Izinkan popup/tab lalu coba lagi.');
            return;
        }
        setPopupLoadingScreen(cameraPopup);

        if (!navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
            showAlert('error', 'Browser ini tidak mendukung akses kamera. Gunakan Chrome/Edge terbaru.');
            try { if (cameraPopup && !cameraPopup.closed) cameraPopup.close(); } catch (e) {}
            cameraPopup = null;
            return;
        }

        try {
            cameraPopup.location.href = popupUrl;
            startPolling();
        } catch (e) {
            showAlert('error', 'Gagal membuka halaman kamera tab baru.');
            stopPolling();
            try { if (cameraPopup && !cameraPopup.closed) cameraPopup.close(); } catch (err) {}
            cameraPopup = null;
        }
    }

    function notifyParentFromPopup(type, message) {
        try {
    ...
    function renderPopupCameraError(message) {
        document.body.className = 'bg-slate-900 text-white';
        document.body.innerHTML = `
            <div class="min-h-screen flex items-center justify-center p-6">
                <div class="max-w-lg w-full bg-slate-800 border border-slate-700 rounded-2xl p-6 text-center">
                    <h2 class="text-lg font-bold mb-2">Kamera Tidak Bisa Diakses</h2>
                    <p class="text-sm text-slate-300 mb-4">${message}</p>
                    <button onclick="window.close()" class="mt-2 bg-red-600 hover:bg-red-700 text-xs font-bold px-4 py-2 rounded-lg">Tutup</button>
                </div>
            </div>
        `;
        notifyParentFromPopup('CAMERA_ERROR', message);
    }
    ...
    function isCameraPopupMode() {
        const params = new URLSearchParams(window.location.search);
        return params.get('camera') === '1';
    }

    function initCameraPopupMode() {
        document.title = 'Scanner Kamera';
        bindBeepUnlockEvents();
        unlockBeepAudio();

        if (!hasSecureCameraContext()) {
            renderPopupCameraError('Browser hanya mengizinkan kamera pada HTTPS atau localhost. Gunakan URL HTTPS / localhost lalu izinkan permission kamera.');
            return;
        }

        if (!navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
            renderPopupCameraError('Browser ini tidak mendukung akses kamera. Gunakan Chrome/Edge versi terbaru.');
            return;
        }
            ...
            <div class="min-h-screen p-4 md:p-6">
                <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div class="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
                        <div class="px-4 py-3 bg-gradient-to-r from-indigo-600 to-blue-600">
                            <h1 class="text-sm font-bold">Kamera Scanner Absensi</h1>
                            <p class="text-[11px] text-indigo-100">Arahkan QR siswa ke kamera</p>
                        </div>
                        <div class="p-4 space-y-3">
                            <div id="qr-reader-popup" class="rounded-xl overflow-hidden bg-black"></div>
                            <p id="popupScanStatus" class="text-xs text-slate-300 bg-slate-700/60 px-3 py-2 rounded-lg">Memuat kamera...</p>
                            <div class="grid grid-cols-2 gap-2">
                                <button id="btnSwitchPopupCam" class="bg-slate-700 hover:bg-slate-600 text-xs font-bold py-2.5 rounded-lg transition">
                                    Ganti Kamera
                                </button>
                                <button id="btnClosePopupCam" class="bg-red-600 hover:bg-red-700 text-xs font-bold py-2.5 rounded-lg transition">
                                    Tutup
                                </button>
                            </div>
                        </div>
                    </div>
                    <div class="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col min-h-[520px]">
                        <div class="px-4 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 flex items-center justify-between">
                            <div>
                                <h2 class="text-sm font-bold">Daftar Absensi</h2>
                                <p class="text-[11px] text-emerald-100">Hasil scan langsung tampil di sini</p>
                            </div>
                            <span id="popupScanCountBadge" class="px-2.5 py-1 rounded-full text-[11px] font-bold bg-white/20 border border-white/25">0 Siswa</span>
                        </div>
                        <div class="overflow-auto flex-1">
                            <table class="w-full text-xs">
                                <thead class="sticky top-0 bg-slate-700/95 border-b border-slate-600">
                                    <tr>
                                        <th class="px-2 py-2 text-center w-8 text-slate-300">#</th>
                                        <th class="px-2 py-2 text-left text-slate-300">Nama</th>
                                        <th class="px-2 py-2 text-center text-slate-300">Kelas</th>
                                        <th class="px-2 py-2 text-center text-slate-300">Status</th>
                                        <th class="px-2 py-2 text-center text-slate-300">Jam</th>
                                    </tr>
                                </thead>
                                <tbody id="popupScanTableBody">
                                    <tr id="popupScanEmptyRow">
        ...
        const statusEl = document.getElementById('popupScanStatus');
        const btnSwitch = document.getElementById('btnSwitchPopupCam');
        const btnClose = document.getElementById('btnClosePopupCam');
        const popupScanTableBody = document.getElementById('popupScanTableBody');
        const popupScanCountBadge = document.getElementById('popupScanCountBadge');
        let popupScanCount = 0;
        const popupScanMap = {};

        const updatePopupScanBadge = () => {
            if (!popupScanCountBadge) return;
            popupScanCountBadge.textContent = `${popupScanCount} Siswa`;
        };

        const createPopupEmptyRow = () => {
            if (!popupScanTableBody) return;
            popupScanTableBody.innerHTML = `
                <tr id="popupScanEmptyRow">
                    <td colspan="5" class="px-4 py-10 text-center text-slate-400">Belum ada absensi masuk.</td>
                </tr>
            `;
            popupScanCount = 0;
            Object.keys(popupScanMap).forEach((key) => delete popupScanMap[key]);
            updatePopupScanBadge();
        };

        const reindexPopupRows = () => {
            if (!popupScanTableBody) return;
            const rows = popupScanTableBody.querySelectorAll('tr[data-nisn]');
            rows.forEach((row, index) => {
                const indexCell = row.querySelector('td');
                if (indexCell) indexCell.textContent = String(index + 1);
            });
            popupScanCount = rows.length;
            updatePopupScanBadge();
        };

        const upsertPopupScanRow = (item) => {
            if (!popupScanTableBody || !item || !item.nisn) return;

            const nisn = String(item.nisn).trim();
            if (!nisn) return;

            const emptyRow = document.getElementById('popupScanEmptyRow');
            if (emptyRow) emptyRow.remove();

            const jenis = String(item.type || 'datang').toLowerCase() === 'pulang' ? 'Pulang' : 'Masuk';
            const jenisClass = jenis === 'Pulang'
                ? 'bg-blue-500/20 text-blue-100 border border-blue-400/40'
                : 'bg-emerald-500/20 text-emerald-100 border border-emerald-400/40';
            const jam = item.jam || '-';

            let row = popupScanMap[nisn] || null;
            if (!row) {
                row = document.createElement('tr');
                row.className = 'border-b border-slate-700/70';
                row.setAttribute('data-nisn', nisn);
                row.innerHTML = `
                    <td class="px-2 py-2 text-center text-slate-400 font-mono"></td>
                    <td class="px-2 py-2">
                        <div class="font-semibold text-slate-100">${item.nama || '-'}</div>
                        <div class="text-[10px] text-slate-400 font-mono">${nisn}</div>
                    </td>
                    <td class="px-2 py-2 text-center">
                        <span class="inline-flex items-center px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-100 border border-indigo-400/40">${item.kelas || '-'}</span>
                    </td>
                    <td class="px-2 py-2 text-center">
                        <span data-popup-type class="inline-flex items-center px-1.5 py-0.5 rounded ${jenisClass}">${jenis}</span>
                    </td>
                    <td class="px-2 py-2 text-center font-mono text-slate-200" data-popup-jam>${jam}</td>
                `;
                popupScanTableBody.appendChild(row);
                popupScanMap[nisn] = row;
            } else {
                const typeNode = row.querySelector('[data-popup-type]');
                const jamNode = row.querySelector('[data-popup-jam]');
                if (typeNode) {
                    typeNode.className = `inline-flex items-center px-1.5 py-0.5 rounded ${jenisClass}`;
                    typeNode.textContent = jenis;
                }
                if (jamNode) jamNode.textContent = jam;
            }

            row.classList.remove('bg-slate-700/40');
            row.classList.add('bg-slate-700/40');
            setTimeout(() => row.classList.remove('bg-slate-700/40'), 1000);
            reindexPopupRows();
        };

        const removePopupScanRow = (nisn) => {
            const key = String(nisn || '').trim();
            if (!key) return;
            const row = popupScanMap[key];
            if (!row) return;
            row.remove();
            delete popupScanMap[key];
            reindexPopupRows();
            if (popupScanCount === 0) createPopupEmptyRow();
        };

        const syncPopupQueueRows = (items) => {
            createPopupEmptyRow();
            if (!Array.isArray(items) || items.length === 0) return;
            items.forEach((item) => upsertPopupScanRow(item));
        };

        const notifyParent = (type, message) => notifyParentFromPopup(type, message);

        const setStatus = (text, tone = 'info') => {
            if (!statusEl) return;
            statusEl.textContent = text;
            if (tone === 'error') statusEl.className = 'text-xs text-red-100 bg-red-600/70 px-3 py-2 rounded-lg';
            else if (tone === 'success') statusEl.className = 'text-xs text-emerald-100 bg-emerald-600/70 px-3 py-2 rounded-lg';
            else statusEl.className = 'text-xs text-slate-200 bg-slate-700/60 px-3 py-2 rounded-lg';

            if (tone === 'error') notifyParent('CAMERA_ERROR', text);
            else notifyParent('CAMERA_INFO', text);
        };

        const ensurePopupToastStyle = () => {
            if (document.getElementById('popup-camera-toast-style')) return;

            const style = document.createElement('style');
            style.id = 'popup-camera-toast-style';
            style.textContent = `
                .swal2-container.swal2-top-end > .swal2-popup.swal2-toast.camera-toast-popup {
                ...
                .swal2-container.swal2-top-end > .swal2-popup.swal2-toast.camera-toast-popup .swal2-title.camera-toast-title {
                    margin: 0 !important;
                    padding: 0 !important;
                    font-size: 12px;
                    font-weight: 600;
                ...
                .swal2-container.swal2-top-end > .swal2-popup.swal2-toast.camera-toast-popup .swal2-icon {
                    margin: 0 .45rem 0 0;
                    transform: scale(.78);
                    align-self: center !important;
                }

                .swal2-container.swal2-top-end > .swal2-popup.swal2-toast.camera-toast-popup .swal2-timer-progress-bar {
                    height: 2px;
                    opacity: .35;
                }
            `;
        ...
        const showPopupCameraToast = (tone, text) => {
            if (typeof Swal === 'undefined') {
                return;
            }

            ensurePopupToastStyle();
            ...
            Swal.fire({
                ...
                customClass: {
                    popup: 'camera-toast-popup',
                    title: 'camera-toast-title',
                },
            });
        };
        ...
        const onScan = (decodedText) => {
            ...
            const now = Date.now();
            if (now - popupLastScanAt < 1500) return;
            popupLastScanAt = now;

           
            setStatus(`QR terdeteksi: ${code}`, 'success');
            try {
                if (window.opener && !window.opener.closed) {
                    window.opener.postMessage({ type: 'QR_SCAN_RESULT', nisn: code, ts: now }, '*');
                }
            } catch (e) {}
        };

        const startCamera = async (index) => {
            if (!popupCameraList.length) {
                setStatus('Kamera tidak ditemukan.', 'error');
                return;
            }

            popupCameraIndex = ((index % popupCameraList.length) + popupCameraList.length) % popupCameraList.length;
            const camera = popupCameraList[popupCameraIndex];

            try {
                if (!popupQrInstance) {
                    popupQrInstance = new Html5Qrcode('qr-reader-popup');
                } else {
                    try { await popupQrInstance.stop(); } catch (e) {}
                    try { await popupQrInstance.clear(); } catch (e) {}
                }

                await popupQrInstance.start(
                    { deviceId: { exact: camera.id } },
                    { fps: 12, qrbox: { width: 260, height: 260 } },
                    onScan,
                    () => {}
                );
                setStatus(`Kamera aktif: ${camera.label || 'Camera ' + (popupCameraIndex + 1)}`);
            } catch (err) {
                setStatus(formatCameraErrorMessage(err), 'error');
            }
        };

        btnClose?.addEventListener('click', async () => {
            try { if (popupQrInstance) await popupQrInstance.stop(); } catch (e) {}
            try { if (popupQrInstance) await popupQrInstance.clear(); } catch (e) {}
            window.close();
        });

        btnSwitch?.addEventListener('click', async () => {
            if (popupCameraList.length < 2) {
                setStatus('Hanya ada satu kamera pada perangkat ini.');
                return;
            }
            await startCamera(popupCameraIndex + 1);
        });

        window.addEventListener('beforeunload', () => {
            try { if (popupQrInstance) popupQrInstance.stop(); } catch (e) {}
        });

        window.addEventListener('message', (event) => {
            const data = event?.data || {};
            if (data.type === 'SCAN_SUCCESS') {
                const text = `Berhasil: ${data.nama || ''} ${data.kelas || ''}`.trim();
                setStatus(text, 'success');
                showPopupCameraToast('success', text);
                playSuccessBeep();
            }
            if (data.type === 'SCAN_DUPLICATE') {
                const text = `Duplikat: ${data.nama || 'Sudah terscan'}`;
                setStatus(text);
                showPopupCameraToast('info', text);
            }
            if (data.type === 'SCAN_ERROR') {
                const text = `${data.message || 'NISN tidak ditemukan'}`;
                setStatus(text, 'error');
                showPopupCameraToast('error', text);
            }
            if (data.type === 'SCAN_QUEUE_UPDATE' && data.item) {
                upsertPopupScanRow(data.item);
            }
            if (data.type === 'SCAN_QUEUE_REMOVE') {
                removePopupScanRow(data.nisn);
            }
            if (data.type === 'SCAN_QUEUE_RESET') {
                createPopupEmptyRow();
            }
            if (data.type === 'SCAN_QUEUE_SYNC') {
                syncPopupQueueRows(data.items || []);
            }
        });
        ...
        navigator.mediaDevices.getUserMedia({ video: true, audio: false })
            ...
            .then(async (cameras) => {
                popupCameraList = cameras || [];
                if (!popupCameraList.length) {
                    setStatus('Tidak ada kamera tersedia.', 'error');
                    return;
                }

                const backIndex = popupCameraList.findIndex((cam) => /back|rear|environment/i.test(cam.label || ''));
                await startCamera(backIndex >= 0 ? backIndex : 0);
            })
            .catch((err) => {
                const message = formatCameraErrorMessage(err);
                setStatus(message, 'error');
            });

        notifyParent('CAMERA_READY', 'Kamera siap menerima data antrian.');
    }

    window.addEventListener('message', (event) => {
        const data = event?.data || {};
        if (data.type === 'QR_SCAN_RESULT') {
            onScanSuccess(String(data.nisn || ''), 'camera');
            return;
        }
        ...
        if (data.type === 'CAMERA_ERROR') {
            showAlert('error', data.message || 'Izin kamera bermasalah.');
            return;
        }

        if (data.type === 'CAMERA_INFO') {
            const pollingStatus = document.getElementById('pollingStatus');
            if (pollingStatus && data.message) {
                const textEl = pollingStatus.querySelector('span');
    ...
    function scanFromFile(input) {
        if (activeScanMode !== SCAN_MODE_QR) {
            showAlert('error', 'Mode RFID aktif. Ubah ke mode QR Code untuk scan dari foto.');
        ...
        Html5Qrcode.scanFile(input.files[0], true)
            .then(decodedText => { resDiv.innerHTML = ''; onScanSuccess(decodedText, 'scanner'); })
    ...
    function getManualNisnInput() {
        return document.getElementById('manualNisn');
    }

    function normalizeManualScanValue(value) {
        return String(value ?? '')
            .replace(/[\u0000-\u001f\u007f]/g, '')
            .trim();
    }

    function focusManualNisnInput() {
        const input = getManualNisnInput();
        if (!input) return;
        if (document.activeElement === input) return;
        input.focus({ preventScroll: true });
    }

    function setupUsbScannerKeyboardWedge() {
        const input = getManualNisnInput();
        if (!input || usbScannerWedgeBound) return;
        usbScannerWedgeBound = true;

        // Jaga input tetap siap untuk mode scanner USB (keyboard wedge).
        setTimeout(() => focusManualNisnInput(), 120);

        document.addEventListener('keydown', (event) => {
            if (event.isComposing || event.ctrlKey || event.metaKey || event.altKey) return;

            const activeEl = document.activeElement;
            const isManualInputActive = activeEl === input;
            ...
            if (event.key === 'Enter') {
                if (isManualInputActive || isTypingElsewhere) {
                    usbScannerBuffer = '';
                    return;
                }

                const payload = normalizeManualScanValue(usbScannerBuffer);
                const isScannerBurst = payload.length >= USB_SCANNER_MIN_LENGTH
                    && now - usbScannerLastKeyAt <= (USB_SCANNER_KEY_GAP_MS * 2);
                ...
                usbScannerBuffer = '';
                submitManualNisn(payload);
                return;
            }

            if (event.key.length !== 1 || isTypingElsewhere) return;

            if (now - usbScannerLastKeyAt > USB_SCANNER_KEY_GAP_MS) {
                usbScannerBuffer = '';
            }
            usbScannerLastKeyAt = now;
            usbScannerBuffer += event.key;

            if (usbScannerBuffer.length > 256) {
                usbScannerBuffer = usbScannerBuffer.slice(-256);
            }
        }, true);
    }

    function submitManualNisn(overrideValue = null) {
        const input = getManualNisnInput();
        const rawValue = overrideValue === null ? (input?.value ?? '') : overrideValue;
        const nisn = normalizeManualScanValue(rawValue);
        if (!nisn) {
            showAlert('error', isRfidMode() ? 'Masukkan UID RFID terlebih dahulu' : 'Masukkan NISN terlebih dahulu');
            return;
        }
        if (input) input.value = '';
        usbScannerBuffer = '';
        onScanSuccess(nisn, 'scanner');
        setTimeout(() => focusManualNisnInput(), 80);
    }

    function showScanErrorPopup(message) {
        Swal.fire({
            icon: 'error',
            title: 'Scan Gagal',
            text: String(message || 'Validasi scan gagal.'),
            timer: 2000,
            timerProgressBar: true,
            showConfirmButton: false,
        });
    ...
    function onScanSuccess(decodedText, source = 'scanner') {
        if (!decodedText || decodedText.trim() === "" || decodedText === "undefined") return;
        if (isScanning) return;
        const scanSource = String(source || 'scanner').toLowerCase() === 'camera' ? 'camera' : 'scanner';

        const key = String(decodedText).trim();
        const now = Date.now();
        const lastSubmit = scanLastSubmitAt[key] || 0;
        if (now - lastSubmit < SCAN_RESUBMIT_COOLDOWN_MS) {
            if (scanLiveMap[key]) {
            ...
            if (scanSource === 'camera' && cameraPopup && !cameraPopup.closed) {
                ...
                try {
                    cameraPopup.postMessage({
                        type: 'SCAN_DUPLICATE',
                        nisn: key,
                        nama: namaEl ? namaEl.textContent : key,
                        kelas: kelasEl ? kelasEl.textContent : ''
                    }, '*');
                } catch(e) {}
            }
            return;
        ...
        // Kirim langsung ke server: validasi + simpan absensi real-time
        const myRole  = currentUser ? role  : '';
        const myKelas = currentUser ? currentUser.kelas : '';
        const scanModeAtSubmit = activeScanMode;
        const isRfidSubmit = scanModeAtSubmit === SCAN_MODE_RFID;

        const runMethod = actionRunner
            .withSuccessHandler(result => {
                isScanning = false;
                const item = Array.isArray(result?.results) ? result.results[0] : null;

                if (result?.success && item?.success) {
                    scanLastSubmitAt[key] = Date.now();
                    const jam = item.jamDatang || item.jamPulang || new Date().toLocaleTimeString('id-ID', {
                        hour: '2-digit',
                        minute: '2-digit',
                        second: '2-digit',
                    });
                    ...
                    if (scanSource === 'camera' && cameraPopup && !cameraPopup.closed) {
                        try {
                            cameraPopup.postMessage({
                                ...
                                kelas: item.kelas || '-',
                                jam,
                                scanType: item.type || 'datang',
                            }, '*');
                        } catch(e) {}
                    }
                } else {
                    const failMessage = item?.message || result?.message || (isRfidSubmit
                        ? 'Kartu RFID tidak ditemukan / scan ditolak.'
                        : 'NISN tidak ditemukan / scan ditolak.');
                    if (scanSource === 'camera' && cameraPopup && !cameraPopup.closed) {
                        try { cameraPopup.postMessage({ type: 'SCAN_ERROR', message: failMessage }, '*'); } catch(e) {}
                    } else {
                        showScanErrorPopup(failMessage);
                    }
                }
            })
            .withFailureHandler(err => {
        ...
        if (isRfidSubmit) {
            runMethod.scanRfidAbsensi(key, myRole, myKelas);
            return;
        }

        runMethod.batchScanAbsensi([key], myRole, myKelas);
    }
    ...
    function syncQueueToCameraTab(type, payload = {}) {
        if (!cameraPopup || cameraPopup.closed) return;
        try {
            cameraPopup.postMessage({ type, ...payload }, '*');
        } catch (e) {}
    }
    ...
    function collectQueueSnapshot() {
        ...
        return Array.from(rows).map((row) => ({
            nisn: row.getAttribute('data-nisn') || '',
            nama: row.getAttribute('data-nama') || '',
            kelas: row.getAttribute('data-kelas') || '',
            type: row.getAttribute('data-type') || 'datang',
            jam: row.getAttribute('data-jam') || '',
        }));
    }
    ...
    function addToScanQueue(siswa) {
        ...
        const now = new Date();
        const jamScan = siswa.jam || now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
        const jenisScan = siswa.type === 'pulang' ? 'Pulang' : 'Masuk';
        const jenisScanClass = siswa.type === 'pulang'
            ? 'bg-blue-100 text-blue-700 border border-blue-200'
            : 'bg-emerald-100 text-emerald-700 border border-emerald-200';
        let tr = scanLiveMap[siswa.nisn] || null;
        if (!tr) {
            scanLiveCount++;
            ...
            tr.innerHTML = `
                ...
                <td class="px-3 py-3 text-center">
                    <span data-scan-type class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${jenisScanClass}">${jenisScan}</span>
                </td>
                <td class="px-3 py-3 text-center">
                    <div data-scan-time class="text-xs font-mono text-gray-600">${jamScan}</div>
                </td>
                <td class="px-3 py-3 text-center">
            ...
            const typeNode = tr.querySelector('[data-scan-type]');
            const timeNode = tr.querySelector('[data-scan-time]');
            if (typeNode) {
                typeNode.className = `inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${jenisScanClass}`;
                typeNode.textContent = jenisScan;
            }
            if (timeNode) {
                timeNode.textContent = jamScan;
            }
        }

        tr.setAttribute('data-nama', siswa.nama || '');
        tr.setAttribute('data-kelas', siswa.kelas || '');
        tr.setAttribute('data-type', siswa.type || 'datang');
        tr.setAttribute('data-jam', jamScan || '');

        playSuccessBeep();
        syncQueueToCameraTab('SCAN_QUEUE_UPDATE', {
            item: {
                nisn: siswa.nisn || '',
                nama: siswa.nama || '',
                kelas: siswa.kelas || '',
                type: siswa.type || 'datang',
                jam: jamScan || '',
            }
        });
    ...
    function removeScanRow(btn, nisn) {
        ...
        delete scanLastSubmitAt[nisn];
        syncQueueToCameraTab('SCAN_QUEUE_REMOVE', { nisn });
    ...
    function resetScanTable() {
        ...
        syncQueueToCameraTab('SCAN_QUEUE_RESET');
        updateBatchBtn();
    }
    ...
    function getSiswaNomorKartuInput() {
        const shell = getModalShell(false);
        if (!shell || shell.classList.contains('hidden')) return null;
        return shell.querySelector('input[name="nomorKartu"]');
    }

    function highlightSiswaNomorKartuInput(input) {
        if (!input) return;
        input.classList.add('ring-2', 'ring-emerald-300');
        setTimeout(() => {
            input.classList.remove('ring-2', 'ring-emerald-300');
        }, 900);
    }

    function fillSiswaNomorKartuFromScanner(rawValue) {
        const input = getSiswaNomorKartuInput();
        if (!input) return;
        const normalized = normalizeManualScanValue(rawValue).toUpperCase();
        if (!normalized) return;
        input.value = normalized;
        input.dispatchEvent(new Event('input', { bubbles: true }));
        highlightSiswaNomorKartuInput(input);
    }

    function focusSiswaNomorKartuInput() {
        const input = getSiswaNomorKartuInput();
        if (!input) return;
        input.focus({ preventScroll: true });
        input.select();
    ...
    function setupSiswaCardUsbKeyboardWedge() {
        if (siswaCardUsbWedgeBound) return;
        siswaCardUsbWedgeBound = true;

        document.addEventListener('keydown', (event) => {
            if (event.isComposing || event.ctrlKey || event.metaKey || event.altKey) return;

            const cardInput = getSiswaNomorKartuInput();
            if (!cardInput) {
                siswaCardUsbBuffer = '';
                return;
            }
            ...
            const activeEl = document.activeElement;
            const isCardInputActive = activeEl === cardInput;
            const isTypingElsewhere = !!(
                activeEl &&
                activeEl !== cardInput &&
                (
            ...
            if (event.key === 'Enter') {
                if (isCardInputActive) {
                    const value = normalizeManualScanValue(cardInput.value).toUpperCase();
                    if (value !== '') {
                        event.preventDefault();
                        cardInput.value = value;
                        highlightSiswaNomorKartuInput(cardInput);
                    }
                    siswaCardUsbBuffer = '';
                    return;
                }
                ...
                const payload = normalizeManualScanValue(siswaCardUsbBuffer).toUpperCase();
                const isScannerBurst = payload.length >= SISWA_CARD_USB_MIN_LENGTH
                    && now - siswaCardUsbLastKeyAt <= (SISWA_CARD_USB_KEY_GAP_MS * 2);
                ...
                event.preventDefault();
                siswaCardUsbBuffer = '';
                fillSiswaNomorKartuFromScanner(payload);
                return;
            }

            if (event.key.length !== 1 || isTypingElsewhere || isCardInputActive) return;

            if (now - siswaCardUsbLastKeyAt > SISWA_CARD_USB_KEY_GAP_MS) {
                siswaCardUsbBuffer = '';
            }
            siswaCardUsbLastKeyAt = now;
            siswaCardUsbBuffer += event.key;

            if (siswaCardUsbBuffer.length > 256) {
                siswaCardUsbBuffer = siswaCardUsbBuffer.slice(-256);
            }
        }, true);
    }
            ...
            <div class="p-6 max-h-[75vh] overflow-y-auto">
                <form onsubmit="saveSiswa(event, ${isEdit})" class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="md:col-span-2">
                            <label class="${labelClass}">Nomor Kartu (RFID)</label>
                            <div class="flex gap-2">
                                <input type="text" name="nomorKartu"
                                    value="${s?.nomorKartu || ''}"
                                    class="${inputClass} font-mono uppercase tracking-wide"
                                    placeholder="Scan/ketik UID RFID..."
                                    autocomplete="off" autocapitalize="off" spellcheck="false"
                                    oninput="this.value = this.value.toUpperCase()">
                                <button type="button" onclick="focusSiswaNomorKartuInput()"
                                    class="shrink-0 inline-flex items-center gap-2 px-3 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 text-xs font-bold hover:bg-emerald-100 transition">
                                    <i class="fas fa-id-card"></i> Scan USB
                                </button>
                            </div>
                            <p class="text-[10px] text-gray-400 mt-1.5">Reader USB mode HID keyboard-wedge: tempel kartu, UID akan terisi otomatis.</p>
                        </div>
            ...
            <div class="p-6 max-h-[70vh] overflow-y-auto">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        ${item('Nomor Kartu RFID', s.nomorKartu, 'fa-id-card')}
                    </div>
                </div>
...
function handleArchiveAndReset() {
    ...
    function bootstrapCurrentPage() {
        ...
        setupSiswaCardUsbKeyboardWedge();

        if (document.getElementById('manualNisn')) {
            setupUsbScannerKeyboardWedge();
            setScannerMode(SCAN_MODE_QR);
        }
