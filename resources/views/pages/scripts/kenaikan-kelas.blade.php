...
function setArchiveTriggerLoading(button, isLoading) {
    if (!button) return;
    ...
    if (isLoading) {
        ...
        button.innerHTML = '<i class="fas fa-circle-notch fa-spin mr-1"></i> Memuat...';
    } else {
        button.disabled = false;
        button.classList.remove('opacity-80', 'cursor-not-allowed');
        button.innerHTML = button.dataset.originalHtml || '<i class="fas fa-archive mr-1"></i> Tutup Tahun (Arsip)';
    }
}

function showModalArsip(evt) {
    const triggerBtn = evt?.currentTarget || document.getElementById('btn-tutup-tahun-arsip');
    setArchiveTriggerLoading(triggerBtn, true);

    actionRunner.withSuccessHandler(async (previewRes) => {
        setArchiveTriggerLoading(triggerBtn, false);
        ...
        const siswaCount = Number(preview.siswaCount || 0);
        const kelasCount = Number(preview.kelasCount || 0);
        const dateRange = String(preview.absensiDateRange || '-');
        const defaultName = String(preview.defaultArchiveName || '').trim();
        const canArchive = preview.canArchive !== false && absensiCount > 0;
        const cannotArchiveMessage = String(preview.cannotArchiveMessage || 'Data absensi masih kosong. Tutup Tahun Ajaran tidak dapat dilakukan.');

        if (!canArchive) {
            Swal.fire('Tidak Bisa Diproses', cannotArchiveMessage, 'info');
            return;
        }

        const step1 = await Swal.fire({
            title: 'Tutup Tahun Ajaran (1/2)',
            width: 620,
            customClass: {
                popup: 'archive-swal-popup',
                confirmButton: 'archive-swal-confirm',
                cancelButton: 'archive-swal-cancel'
            },
            html: `
                <div style="text-align:left;">
                    <p style="font-size:12px;color:#6b7280;font-weight:600;margin:0 0 6px 0;">Preview data sebelum proses arsip:</p>
                    <div class="archive-preview-grid">
                        <div class="archive-preview-card">
                            <span class="label">Data Absensi</span>
                            <span class="value">${absensiCount.toLocaleString('id-ID')} baris</span>
                        </div>
                        <div class="archive-preview-card">
                            <span class="label">Rentang Tanggal</span>
                            <span class="value">${dateRange}</span>
                        </div>
                        <div class="archive-preview-card">
                            <span class="label">Data Siswa</span>
                            <span class="value">${siswaCount.toLocaleString('id-ID')} baris</span>
                        </div>
                        <div class="archive-preview-card">
                            <span class="label">Data Kelas</span>
                            <span class="value">${kelasCount.toLocaleString('id-ID')} baris</span>
                        </div>
                    </div>
                    <div class="archive-preview-warning">
                        Setelah arsip berhasil dibuat, data absensi dalam rentang tanggal arsip akan dihapus dari sistem berjalan.
                    </div>
                </div>
                <label for="swal-input-arsip" style="display:block;text-align:left;font-size:12px;font-weight:700;color:#374151;margin-top:10px;">
                    Nama Arsip
                </label>
            ...
            preConfirm: () => {
                const nama = Swal.getPopup().querySelector('#swal-input-arsip').value.trim();
                if (!nama) {
                    Swal.showValidationMessage('Nama arsip wajib diisi');
                    return false;
                }
                return nama;
            }
        ...
        const step2 = await Swal.fire({
            ...
            html: `
                <div class="text-left text-sm text-gray-700">
                    <p class="mb-2">Anda akan membuat arsip: <b>"${archiveName}"</b>.</p>
                    <p class="mb-3" style="font-size:12px;">
                        Setelah arsip berhasil dibuat, <b class="text-red-600">SEMUA DATA ABSENSI AKTIF</b> serta <b class="text-red-600">HARI LIBUR DALAM RENTANG TANGGAL ARSIP</b> akan dihapus.
                    </p>
                    <p class="text-xs text-gray-500 mb-2">Ketik <b>ARSIP</b> untuk melanjutkan:</p>
                </div>
                <input id="swal-input-confirm-arsip" class="swal2-input" placeholder="Ketik: ARSIP">
            `,
            icon: 'warning',
            showCancelButton: true,
            ...
            preConfirm: () => {
                const confirmText = Swal.getPopup().querySelector('#swal-input-confirm-arsip').value.trim().toUpperCase();
                if (confirmText !== 'ARSIP') {
                    Swal.showValidationMessage('Konfirmasi tidak sesuai. Ketik ARSIP.');
                    return false;
                }
                ...
                return new Promise((resolve, reject) => {
                    actionRunner.withSuccessHandler((res) => {
                        ...
                        reject(new Error(res?.message || 'Gagal mengarsipkan data.'));
                    }).withFailureHandler((err) => {
                        reject(new Error('Terjadi kesalahan server: ' + err));
                    }).archiveAndResetYear(archiveName);
                }).catch((error) => {
                    Swal.showValidationMessage(error.message || 'Gagal mengarsipkan data.');
                });
            }
        });

        if (!step2.isConfirmed) return;
        const arsipRes = step2.value || {};
        Swal.fire(
            'Selesai!',
            `Data absensi berhasil diarsipkan.<br><a href="${arsipRes.url}" target="_blank" class="text-blue-600 underline">Buka Arsip</a>`,
            'success'
        );
    }).withFailureHandler((err) => {
        setArchiveTriggerLoading(triggerBtn, false);
