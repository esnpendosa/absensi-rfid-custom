...
function handleArchiveAndReset() {
    ...
    window.loadQRCodeSiswa = async function loadQRCodeSiswaPatched(nisn, nama, kelas) {
        ...
        const renderedAtLabel = new Date().toLocaleString('id-ID', {
            day: '2-digit',
            month: 'short',
            year: 'numeric',
            hour: '2-digit',
        ...
        const websiteName = String(window.APP_WEBSITE_NAME || '').replace(/\s+/g, ' ').trim() || 'Absensindo';
        container.innerHTML = `
            <div class="mx-auto w-full max-w-md">
                <div class="relative overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-[0_20px_45px_rgba(15,23,42,0.12)]">
                    <div class="pointer-events-none absolute -top-16 -right-16 h-40 w-40 rounded-full bg-indigo-200/50 blur-2xl"></div>
                    <div class="pointer-events-none absolute -bottom-16 -left-16 h-40 w-40 rounded-full bg-sky-200/50 blur-2xl"></div>
                    <div class="relative bg-gradient-to-r from-indigo-700 via-blue-600 to-cyan-500 px-6 py-5 text-white">
                    ...
                    <div class="relative px-6 py-6">
                        <div class="space-y-3">
                            <div class="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Nama</p>
                    ...
                    <div class="relative border-t border-gray-100 bg-gray-50/80 px-6 py-3 text-[11px] text-gray-600">
                        Terbit: <span class="font-semibold text-gray-700">${renderedAtLabel}</span>
                    </div>
                </div>
            </div>
        `;
