<script>
    ...
    const tableState = {
                ...
                monitoring: { fullData: [], filtered: [], limit: 10, page: 1, search: '', classFilter: '', statusFilter: '', serverMode: true, serverMeta: null, requestId: 0, searchTimer: null, attendanceMode: 'masuk_saja' }
            };

function reloadMonitoringTable(debounceMs = 0) {
        const state = tableState.monitoring;
        if (!state) {
            return;
        }
...
function handleTableStatusFilter(type, status) {
        if (tableState[type]) {
            tableState[type].statusFilter = normalizeMonitoringStatusFilter(status);
            tableState[type].page = 1; // Reset ke halaman 1
            if (type === 'monitoring') {
                reloadMonitoringTable();
                return;
...
function normalizeMonitoringStatusFilter(status) {
...
function monitoringStatusMatches(itemStatus, filterStatus) {
    ...
    function handleTableLimit(type, limit) {
        tableState[type].limit = type === 'monitoring'
    ...
    function changePage(type, direction) {
        ...
        if (type === 'monitoring' && state.serverMode) {
...
function processTableData(type) {
        ...
        if (type === 'monitoring' && state.serverMode) {
            ...
            renderMonitoringRows(rows, meta.from > 0 ? meta.from - 1 : 0);
...
function populateAllClassDropdowns() {
    ...
    dropdownIds.forEach(id => {
        ...
        if (select) {
            if (id === 'monitoringKelas') {
                const fixedRoleKelas = (
                    role === 'wakel' ||
                    role === 'siswa' ||
                    (role === 'piket' && String(currentUser?.kelas || '').trim() !== '')
                ) ? String(currentUser?.kelas || '').trim() : '';

                if (fixedRoleKelas) {
                    select.innerHTML = '';
                    const option = document.createElement('option');
                    option.value = fixedRoleKelas;
                    option.text = fixedRoleKelas;
                    select.appendChild(option);
                    select.value = fixedRoleKelas;
                    select.disabled = true;
                    tableState.monitoring.classFilter = fixedRoleKelas;
                    return;
                }
            ...
            if (id === 'monitoringKelas') {
                tableState.monitoring.classFilter = String(select.value || '').trim();
            }
        }
    });
}

function applyMonitoringFiltersFromQuery() {
    if (!document.getElementById('tbody-monitoring')) {
        return;
    }

    const params = new URLSearchParams(window.location.search);
    const status = normalizeMonitoringStatusFilter(params.get('status'));
    if (!status) {
        return;
    }

    tableState.monitoring.statusFilter = status;
    tableState.monitoring.page = 1;

    const statusSelect = document.getElementById('monitoringStatusFilter');
    if (statusSelect) {
...
function exportMonitoringExcel() {
    ...
    // 2. Monitoring diekspor untuk hari berjalan
    const now = new Date();
    ...
    // 4. Siapkan Filter
    const selectedMonitoringClass = String(document.getElementById('monitoringKelas')?.value || '').trim();
    let kelasFilter = selectedMonitoringClass || null;
    if ((role === 'wakel' || role === 'siswa') && currentUser?.kelas) {
        kelasFilter = currentUser.kelas;
    } else if (role === 'piket' && String(currentUser?.kelas || '').trim() !== '') {
        kelasFilter = currentUser.kelas;
    }
...
function canManageMonitoringAttendance() {
    return role === 'admin' || role === 'wakel';
}

function resolveMonitoringKelasFilter() {
    const selectedMonitoringClass = String(document.getElementById('monitoringKelas')?.value || '').trim();
    let kelasFilter = selectedMonitoringClass || null;
    ...
    return kelasFilter ? String(kelasFilter).trim() : '';
}

function syncMonitoringPulangMassalButton() {
    const btn = document.getElementById('btnMonitoringPulangMassal');
    if (!btn) return;

    const isCheckoutMode = String(tableState.monitoring?.attendanceMode || 'masuk_saja') === 'masuk_pulang';
    const canManage = canManageMonitoringAttendance();
    btn.disabled = !canManage || !isCheckoutMode;

    if (!canManage) {
        btn.title = 'Anda tidak memiliki izin mengubah absensi.';
    } else if (!isCheckoutMode) {
        btn.title = 'Aktifkan mode absensi Masuk + Pulang untuk memakai fitur ini.';
    } else {
        const kelas = resolveMonitoringKelasFilter();
        btn.title = kelas ? `Tandai absen pulang untuk kelas ${kelas}` : 'Tandai absen pulang untuk semua kelas';
    }
}

async function confirmMonitoringPulangMassal(kelas) {
    const scopeText = kelas ? `kelas ${kelas}` : 'semua kelas';
    const message = `Tandai semua siswa ${scopeText} yang sudah absen masuk dan belum pulang sebagai sudah absen pulang?`;

    if (window.Swal && typeof Swal.fire === 'function') {
        const result = await Swal.fire({
            title: 'Absen Pulang Massal',
            text: message,
            icon: 'question',
...
function setMonitoringPulangMassalLoading(isLoading) {
    const btn = document.getElementById('btnMonitoringPulangMassal');
    if (!btn) return;
    ...
    syncMonitoringPulangMassalButton();
}

async function markMonitoringPulangMassal() {
    if (!canManageMonitoringAttendance()) {
        showAlert('error', 'Anda tidak memiliki izin mengubah absensi.');
        return;
    }

    const isCheckoutMode = String(tableState.monitoring?.attendanceMode || 'masuk_saja') === 'masuk_pulang';
    if (!isCheckoutMode) {
        showAlert('warning', 'Absen pulang massal hanya tersedia saat mode Masuk + Pulang aktif.');
        return;
    }

    const kelas = resolveMonitoringKelasFilter();
    const confirmed = await confirmMonitoringPulangMassal(kelas);
    if (!confirmed) {
        return;
    }

    setMonitoringPulangMassalLoading(true);

    patchedRun('markPulangMassal', { kelas })
        .then((res) => {
            if (!res.success) throw new Error(res.message || 'Gagal menandai absen pulang.');

            loadMonitoringAbsensi();
            const updated = parseInt(res.updated || 0, 10) || 0;
            if (updated > 0) {
                showAlert('success', res.message || 'Absen pulang massal berhasil.');
            } else {
                showAlert('info', res.message || 'Tidak ada siswa yang perlu ditandai pulang.');
            }
        })
        .catch((err) => {
        ...
        .finally(() => {
            setMonitoringPulangMassalLoading(false);
        });
}
...
function renderMonitoringRows(data, startIdx) {
    ...
    tbody.innerHTML = data.map((d, i) => {
        ...
        else if(d.status === 'Alpa' || d.status === 'Alfa') statusColor = 'bg-red-100 text-red-700';
        const masukOptionHtml = canSelectMasukStatus
            ? `<option value="Masuk" ${d.status === 'Masuk' ? 'selected' : ''}>Masuk</option>`
            : (d.status === 'Masuk'
                ? '<option value="Masuk" selected hidden>Masuk</option>'
                : '');
...
function handleArchiveAndReset() {
    ...
    window.loadMonitoringAbsensi = function loadMonitoringAbsensiPatched() {
        ...
        state.classFilter = String(kelasFilter || '').trim();
        ...
        patchedRun('getMonitoringRealtime', {
            paginated: true,
            page: state.page || 1,
            ...
            .then((res) => {
                if (requestId !== tableState.monitoring.requestId) {
                    return;
                }
                ...
                state.attendanceMode = String(res?.meta?.attendanceMode || state.attendanceMode || 'masuk_saja');
                state.page = parseInt(state.serverMeta.page || state.page || 1, 10) || 1;
                syncMonitoringPulangMassalButton();
                if (Array.isArray(res?.meta?.classes)) {
                    existingClasses = res.meta.classes;
                    populateAllClassDropdowns();
                    syncMonitoringPulangMassalButton();
                }
    ...
    function bootstrapCurrentPage() {
        ...
        applyMonitoringFiltersFromQuery();
