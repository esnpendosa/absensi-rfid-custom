...
function populateAllClassDropdowns() {
    ...
    dropdownIds.forEach(id => {
        ...
        if (select) {
            if (id === 'rekapKelas') {
                const singleClassRole = ['wakel', 'guru', 'siswa'].includes(String(role || '').toLowerCase())
                    || (String(role || '').toLowerCase() === 'piket' && String(currentUser?.kelas || '').trim() !== '');
                const forcedKelas = String(currentUser?.kelas || '').trim();

                if (singleClassRole && forcedKelas) {
                    select.innerHTML = '';
                    const option = document.createElement('option');
                    option.value = forcedKelas;
                    option.text = forcedKelas;
                    select.appendChild(option);
                    select.value = forcedKelas;
...
function renderMonitoringRows(data, startIdx) {
    ...
    tbody.innerHTML = data.map((d, i) => {
        ...
        if(d.status === 'Masuk') statusColor = 'bg-sky-100 text-sky-700';
        else if(d.status === 'Hadir') statusColor = 'bg-green-100 text-green-700';
        ...
        <tr class="hover:bg-gray-50 border-b border-gray-50 transition group">
            ...
            <td class="p-4 text-center relative">
                <select onchange="changeStatus('${d.nisn}', '${d.nama}', '${d.kelas}', this)" 
                    ...
                    <option value="Masuk" ${d.status === 'Masuk' ? 'selected' : ''}>Masuk</option>
...
function changeStatus(nisn, nama, kelas, selectElement) {
    ...
    actionRunner.withSuccessHandler(res => {
        ...
        if (res.success) {
            ...
            if(newStatus === 'Masuk') newColor = 'bg-sky-100 text-sky-700';
            else if(newStatus === 'Hadir') newColor = 'bg-green-100 text-green-700';
...
const rekapBulananState = {
    daysInMonth: 0,
    students: [],
    page: 1,
    limit: 10,
};
let rekapAutoLoadTimer = null;

const rekapFilterState = {
    bulan: new Date().getMonth(),
    tahun: new Date().getFullYear(),
    kelas: '',
};

function setRekapBulananLimit(value) {
    rekapBulananState.limit = value === 'all' ? Infinity : Math.max(1, parseInt(value, 10) || 10);
    rekapBulananState.page = 1;
    renderRekapBulananPage();
}

function changeRekapBulananPage(direction) {
    const total = rekapBulananState.students.length;
    const maxPage = rekapBulananState.limit === Infinity
        ? 1
        : Math.max(1, Math.ceil(total / rekapBulananState.limit));
    const nextPage = rekapBulananState.page + direction;
    if (nextPage < 1 || nextPage > maxPage) return;
    rekapBulananState.page = nextPage;
    renderRekapBulananPage();
}

function updateRekapBulananPagination(total, startIdx, currentCount) {
    const infoEl = document.getElementById('info-rekap-bulanan');
    const btnPrev = document.getElementById('btn-prev-rekap-bulanan');
    const btnNext = document.getElementById('btn-next-rekap-bulanan');
    ...
    infoEl.textContent = `Menampilkan ${startIdx + 1} - ${endIdx} dari ${total} data`;
    btnPrev.disabled = rekapBulananState.page <= 1;
    btnNext.disabled = rekapBulananState.page >= maxPage;
}

function renderRekapBulananPage() {
    const tbody = document.getElementById('tbody-rekap-bulanan');
    if (!tbody) return;

    const students = Array.isArray(rekapBulananState.students) ? rekapBulananState.students : [];
    ...
    const startIdx = rekapBulananState.limit === Infinity
        ? 0
        : (rekapBulananState.page - 1) * rekapBulananState.limit;
    const pagedStudents = rekapBulananState.limit === Infinity
        ? students
        : students.slice(startIdx, startIdx + rekapBulananState.limit);

    let rowsHTML = '';
    pagedStudents.forEach((siswa, idx) => {
        let row = `<tr class="hover:bg-gray-50 border-b border-gray-100 group">`;
        row += `<td class="p-3 text-center text-gray-500 sticky left-0 bg-white group-hover:bg-gray-50 z-10">${startIdx + idx + 1}</td>`;
        ...
        siswa.dailyCodes.forEach(day => {
            ...
            else if (day.code === 'M') colorClass = 'text-sky-600 font-bold bg-sky-50';
    ...
    updateRekapBulananPagination(total, startIdx, pagedStudents.length);
}

function getRekapBulananColspan() {
    const headColumns = document.querySelectorAll('#thead-rekap-bulanan th').length;
    return Math.max(10, headColumns || 0);
}

function getActiveRekapBulananMonth() {
...
function getCurrentRekapFilterSelection() {
    const bulan = getActiveRekapBulananMonth();
    const tahun = getActiveRekapYear();
    const kelasEl = document.getElementById('rekapKelas');

    let kelas = kelasEl ? String(kelasEl.value || '').trim() : '';
    if ((role === 'wakel' || role === 'guru') && currentUser && currentUser.kelas) {
        kelas = String(currentUser.kelas).trim();
    }

    rekapFilterState.bulan = Number.isFinite(parseInt(bulan, 10)) ? parseInt(bulan, 10) : new Date().getMonth();
    rekapFilterState.tahun = Number.isFinite(parseInt(tahun, 10)) ? parseInt(tahun, 10) : new Date().getFullYear();
    rekapFilterState.kelas = kelas;

    return {
        bulan: rekapFilterState.bulan,
        tahun: rekapFilterState.tahun,
        kelas: rekapFilterState.kelas,
    };
}

function bindRekapFilterSync() {
    ['rekapKelas', 'rekapTahun', 'rekapBulan'].forEach((id) => {
        const el = document.getElementById(id);
        if (!el || el.dataset.rekapSyncBound === '1') return;
        el.dataset.rekapSyncBound = '1';
        el.addEventListener('change', () => {
            getCurrentRekapFilterSelection();
            if (typeof loadDataRekapBulanan === 'function') {
                if (rekapAutoLoadTimer) {
                    clearTimeout(rekapAutoLoadTimer);
                }
                rekapAutoLoadTimer = setTimeout(() => {
...
function updateRekapBulanBerjalanLabel() {
    const label = document.getElementById('rekap-bulan-berjalan-label');
    const now = new Date();
    if (label) {
...
function loadDataRekapBulanan() {
    const currentFilters = getCurrentRekapFilterSelection();
    const bulan = currentFilters.bulan;
    const tahun = currentFilters.tahun;
    const kelas = currentFilters.kelas;
    const limitSelect = document.getElementById('rekapBulananLimit');
    if (limitSelect) {
        rekapBulananState.limit = limitSelect.value === 'all'
            ? Infinity
            : Math.max(1, parseInt(limitSelect.value, 10) || 10);
    }
    rekapBulananState.page = 1;
    ...
    tbody.innerHTML = `<tr><td colspan="${getRekapBulananColspan()}" class="p-8 text-center"><i class="fas fa-circle-notch fa-spin mr-2"></i>Mengambil data...</td></tr>`;

    actionRunner.withSuccessHandler(res => {
        if(!res.success) {
            tbody.innerHTML = `<tr><td colspan="${getRekapBulananColspan()}" class="p-4 text-center text-red-500">${res.message}</td></tr>`;
            rekapBulananState.students = [];
            rekapBulananState.page = 1;
            updateRekapBulananPagination(0, 0, 0);
            return;
        }
        ...
        rekapBulananState.daysInMonth = data.daysInMonth || 0;
        rekapBulananState.students = Array.isArray(data.students) ? data.students : [];
        renderRekapBulananPage();

    }).withFailureHandler(err => {
        tbody.innerHTML = `<tr><td colspan="${getRekapBulananColspan()}" class="p-4 text-center text-red-500">Gagal memuat data: ${err}</td></tr>`;
        rekapBulananState.students = [];
        rekapBulananState.page = 1;
        updateRekapBulananPagination(0, 0, 0);
    }).getMonthlyReportData(parseInt(bulan, 10), parseInt(tahun, 10), kelas);
}
...
function handleArchiveAndReset() {
    ...
    window.renderRekapRows = function renderRekapRowsPatched(data) {
        ...
        tbody.innerHTML = data.map((row, idx) => {
            ...
            if (row.status === 'Masuk') statusClass = 'bg-sky-100 text-sky-700';
            else if (row.status === 'Hadir') statusClass = 'bg-green-100 text-green-700';
...
function bootstrapCurrentPage() {
        ...
        if (document.getElementById('tbody-rekap-bulanan')) {
