// ==========================================================================
    ...
    const ATTENDANCE_MODE_CHECKIN_ONLY = 'masuk_saja';
    const ATTENDANCE_MODE_CHECKIN_CHECKOUT = 'masuk_pulang';
...
function loadAdminDashboard() {
    ...
    function buildAttendanceAreaGradient(canvas, startColor, endColor) {
        const context = canvas.getContext('2d');
        const gradient = context.createLinearGradient(0, 0, 0, canvas.clientHeight || 320);

        gradient.addColorStop(0, startColor);
        gradient.addColorStop(0.6, endColor);
        gradient.addColorStop(1, 'rgba(255, 255, 255, 0)');

        return gradient;
    }

    function ensureDashboardChartPluginsRegistered() {
    ...
    function safelyRenderDashboardChart(callback) {
        try {
            callback();
        } catch (error) {
    ...
    function normalizeDashboardAttendanceMode(mode) {
        return String(mode || '').trim().toLowerCase() === ATTENDANCE_MODE_CHECKIN_CHECKOUT
            ? ATTENDANCE_MODE_CHECKIN_CHECKOUT
            : ATTENDANCE_MODE_CHECKIN_ONLY;
    }

    function buildAttendanceModeMeta(mode) {
        if (mode === ATTENDANCE_MODE_CHECKIN_CHECKOUT) {
            return {
                pillLabel: 'Masuk + Pulang',
                label: 'Masuk + Pulang',
                hint: 'Status Masuk baru berubah menjadi Hadir setelah siswa menuntaskan absensi pulang.',
                pillClass: 'border-sky-200 bg-sky-50 text-sky-700',
            };
        }

        return {
            pillLabel: 'Masuk Saja',
            label: 'Masuk Saja',
            hint: 'Siswa langsung dihitung hadir saat absensi masuk berhasil tercatat.',
            pillClass: 'border-emerald-200 bg-emerald-50 text-emerald-700',
        };
    }

    function buildAttendanceChartSeries(mode, counts) {
        if (mode === ATTENDANCE_MODE_CHECKIN_CHECKOUT) {
            return {
                labels: ['Hadir', 'Masuk', 'Sakit', 'Izin', 'Alpa', 'Belum Absen'],
                values: [counts.hadir, counts.masuk, counts.sakit, counts.izin, counts.alpa, counts.belum],
                pointColors: ['#10B981', '#0EA5E9', '#F59E0B', '#3B82F6', '#EF4444', '#9CA3AF'],
            };
        }

        return {
            labels: ['Hadir', 'Sakit', 'Izin', 'Alpa', 'Belum Absen'],
            values: [counts.hadir, counts.sakit, counts.izin, counts.alpa, counts.belum],
            pointColors: ['#10B981', '#F59E0B', '#3B82F6', '#EF4444', '#9CA3AF'],
        };
    }

    function applyOperationalDashboardState(mode, pendingCheckoutCount) {
        const meta = buildAttendanceModeMeta(mode);
        const pendingCount = mode === ATTENDANCE_MODE_CHECKIN_CHECKOUT ? pendingCheckoutCount : 0;
        const pendingHint = mode === ATTENDANCE_MODE_CHECKIN_CHECKOUT
            ? (pendingCount > 0
                ? `${pendingCount} siswa sudah absen masuk dan masih menunggu absensi pulang.`
                : 'Tidak ada siswa yang masih menunggu absensi pulang hari ini.')
            : 'Mode masuk saja aktif, jadi absensi pulang tidak dipakai hari ini.';

        [
            { prefix: 'admin' },
            { prefix: 'guru' },
        ].forEach(({ prefix }) => {
            patchedSetText(`${prefix}AttendanceModeLabel`, meta.label, meta.label);
            patchedSetText(`${prefix}AttendanceModeHint`, meta.hint, meta.hint);
            patchedSetText(`${prefix}PendingCheckoutCount`, pendingCount, '0');
            patchedSetText(`${prefix}PendingCheckoutHint`, pendingHint, pendingHint);

            const pill = document.getElementById(`${prefix}AttendanceModePill`);
            if (pill) {
                pill.className = `inline-flex items-center rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wide border ${meta.pillClass}`;
                pill.textContent = meta.pillLabel;
            }
        });
    }

    function createAttendanceAreaChart(canvas, chartLabel, series, theme = {}) {
        if (typeof Chart === 'undefined') {
            throw new Error('Chart.js tidak termuat.');
        }

        const pointColors = Array.isArray(series?.pointColors) && series.pointColors.length
            ? series.pointColors
            : (Array.isArray(theme.pointColors) && theme.pointColors.length
                ? theme.pointColors
                : ['#10B981', '#F59E0B', '#3B82F6', '#EF4444', '#9CA3AF']);
        const lineColor = theme.lineColor || '#4F46E5';
        const labelColor = theme.labelColor || '#4338CA';
        const context = canvas.getContext('2d');

        if (!context) {
            throw new Error('Canvas chart tidak tersedia.');
        }

        ensureDashboardChartPluginsRegistered();
        ...
        return new Chart(context, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: buildAttendanceAreaGradient(
                        canvas,
                        theme.fillTop || 'rgba(79, 70, 229, 0.30)',
                        theme.fillBottom || 'rgba(129, 140, 248, 0.10)'
                    ),
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: (context) => {
                                const parsedValue = typeof context?.parsed === 'object' && context.parsed !== null
                                    ? context.parsed.y
                                    : context?.raw;

                                return `${context.label}: ${parsedValue ?? 0}`;
                            }
                        }
                    },
                    datalabels: {
    ...
    function renderAdminChart(mode, counts) {
        ...
        adminChartInstance = createAttendanceAreaChart(canvas, 'Jumlah Siswa', buildAttendanceChartSeries(mode, counts), {
            lineColor: '#4F46E5',
            fillTop: 'rgba(79, 70, 229, 0.32)',
            fillBottom: 'rgba(129, 140, 248, 0.10)',
            labelColor: '#4338CA'
        });
    }

    function renderStudentChart(mode, counts) {
        ...
        studentChartInstance = createAttendanceAreaChart(canvas, 'Jumlah Siswa', buildAttendanceChartSeries(mode, counts), {
            lineColor: '#0EA5E9',
            fillTop: 'rgba(14, 165, 233, 0.28)',
            fillBottom: 'rgba(125, 211, 252, 0.10)',
            labelColor: '#0369A1'
        });
    }
...
function loadGuruDashboard() {
    ...
    function renderGuruChart(mode, counts) {
        const canvas = document.getElementById('guruAttendanceChart');
        if (!canvas) return;
        ...
        guruChartInstance = createAttendanceAreaChart(canvas, 'Jumlah Siswa', buildAttendanceChartSeries(mode, counts), {
            lineColor: '#2563EB',
            fillTop: 'rgba(37, 99, 235, 0.30)',
            fillBottom: 'rgba(96, 165, 250, 0.10)',
            labelColor: '#1D4ED8'
        });
    }
...
function handleArchiveAndReset() {
    ...
    function patchedStudentStatusTheme(status) {
        const rawStatus = String(status || '').trim();
        const normalized = rawStatus === '' ? 'Belum Absen' : rawStatus;
        const key = normalized === 'Alfa' ? 'Alpa' : normalized;

        const themes = {
            'Hadir': {
                label: 'Hadir',
                hint: 'Kehadiran Anda sudah tercatat dengan baik.',
                subtitle: 'Pertahankan disiplin kehadiran setiap hari.',
                badge: 'bg-emerald-400/15 border-emerald-200/40 text-emerald-100',
                hero: ['from-emerald-900', 'via-emerald-800', 'to-teal-900'],
                progress: 'bg-emerald-300',
            },
            'Masuk': {
                label: 'Masuk',
                hint: 'Absensi masuk sudah tercatat. Jangan lupa lakukan absensi pulang.',
                subtitle: 'Selesaikan absensi pulang agar status berubah menjadi hadir penuh.',
                badge: 'bg-sky-300/15 border-sky-200/40 text-sky-100',
                hero: ['from-sky-900', 'via-cyan-800', 'to-indigo-900'],
                progress: 'bg-cyan-300',
            },
            'Izin': {
                label: 'Izin',
                hint: 'Status izin tercatat untuk hari ini.',
                subtitle: 'Pastikan keterangan izin sudah lengkap.',
                badge: 'bg-blue-400/15 border-blue-200/40 text-blue-100',
                hero: ['from-blue-900', 'via-indigo-800', 'to-sky-900'],
                progress: 'bg-blue-300',
            },
            'Sakit': {
                label: 'Sakit',
                hint: 'Status sakit tercatat. Semoga lekas pulih.',
                subtitle: 'Fokus pada pemulihan kesehatan Anda.',
                badge: 'bg-amber-300/15 border-amber-200/40 text-amber-100',
                hero: ['from-amber-900', 'via-orange-800', 'to-slate-900'],
                progress: 'bg-amber-300',
            },
            'Alpa': {
                label: 'Alpa',
                hint: 'Belum ada absensi valid untuk hari ini.',
                subtitle: 'Segera konfirmasi ke guru jika terjadi kesalahan data.',
                badge: 'bg-rose-300/15 border-rose-200/40 text-rose-100',
                hero: ['from-rose-900', 'via-red-800', 'to-slate-900'],
                progress: 'bg-rose-300',
            },
            'Belum Absen': {
                label: 'Belum Absen',
                hint: 'Anda belum melakukan absensi datang hari ini.',
                subtitle: 'Silakan lakukan absensi sesuai jam yang ditentukan.',
                badge: 'bg-white/15 border-white/30 text-white',
                hero: ['from-slate-900', 'via-indigo-900', 'to-slate-900'],
                progress: 'bg-indigo-300',
            },
            'Libur': {
                label: 'Libur',
                hint: 'Hari ini dijadwalkan sebagai hari libur.',
                subtitle: 'Tidak ada kewajiban absensi pada hari libur.',
                badge: 'bg-cyan-300/15 border-cyan-100/40 text-cyan-100',
                hero: ['from-cyan-900', 'via-sky-800', 'to-indigo-900'],
                progress: 'bg-cyan-300',
            },
        };

        return themes[key] || themes['Belum Absen'];
    }

    function patchedTimeToMinutes(timeText) {
    ...
    function patchedPickRandom(items) {
    ...
    function patchedResolveStudentMotivation(status, context = {}) {
        const normalizedStatus = String(status || '').trim() || 'Belum Absen';
        const effectiveStatus = normalizedStatus === 'Alfa' ? 'Alpa' : normalizedStatus;
        const attendanceRate = Number.isFinite(Number(context.attendanceRate)) ? Number(context.attendanceRate) : 0;
        const jamDatangMinutes = patchedTimeToMinutes(context.jamDatang);

        let disciplineScore = attendanceRate;
        if (effectiveStatus === 'Hadir') {
            if (jamDatangMinutes !== null) {
                if (jamDatangMinutes <= 7 * 60) {
                    disciplineScore += 8;
                } else if (jamDatangMinutes <= (7 * 60) + 15) {
                    disciplineScore += 4;
                } else if (jamDatangMinutes >= 8 * 60) {
                    disciplineScore -= 8;
                }
            }
        } else if (effectiveStatus === 'Belum Absen' || effectiveStatus === 'Alpa') {
            disciplineScore -= 20;
        } else if (effectiveStatus === 'Izin' || effectiveStatus === 'Sakit') {
            disciplineScore -= 5;
        }

        let level = 'needs_action';
        if (effectiveStatus === 'Libur') {
            level = 'holiday';
        } else if (effectiveStatus === 'Izin' || effectiveStatus === 'Sakit') {
            level = 'care';
        } else if (effectiveStatus === 'Hadir') {
            if (disciplineScore >= 92) {
                level = 'excellent';
        ...
        const motivationalMessages = {
            excellent: [
                'Disiplin Anda sangat baik hari ini. Pertahankan konsistensi sampai akhir semester.',
                'Luar biasa, kehadiran Anda bisa menjadi teladan bagi teman sekelas.',
                'Anda menjaga ritme belajar dengan sangat baik. Teruskan performa ini.',
                'Komitmen Anda terhadap kehadiran patut diapresiasi. Jaga standar ini setiap hari.',
            ],
            good: [
                'Kedisiplinan Anda sudah bagus. Tingkatkan sedikit lagi agar makin konsisten.',
                'Progres Anda positif. Pertahankan kebiasaan hadir tepat waktu.',
                'Arah kedisiplinan Anda sudah tepat. Teruskan untuk hasil belajar yang lebih baik.',
                'Anda berada di jalur yang baik. Jaga fokus dan ketepatan waktu setiap hari.',
            ],
            fair: [
                'Kehadiran Anda sudah berjalan. Coba tingkatkan ketepatan waktunya.',
                'Langkah Anda sudah baik. Konsistensi kecil setiap hari akan memberi dampak besar.',
                'Ayo tingkatkan disiplin sedikit demi sedikit agar progres belajar lebih stabil.',
                'Anda sudah mulai baik, sekarang waktunya naik ke level disiplin berikutnya.',
            ],
            care: [
                'Jaga kesehatan dan segera kembali ke ritme belajar terbaik Anda.',
                'Semoga kondisi Anda segera membaik. Tetap semangat menjaga proses belajar.',
                'Fokus pemulihan dahulu, lalu lanjutkan lagi dengan disiplin yang konsisten.',
                'Tetap semangat. Setelah pulih, mari kembali dengan performa yang lebih baik.',
            ],
            holiday: [
                'Hari ini libur. Gunakan waktu untuk istirahat atau menyiapkan pelajaran berikutnya.',
                'Selamat menikmati hari libur. Tetap jaga kebiasaan belajar yang positif.',
                'Hari libur bisa jadi momen evaluasi ringan agar besok lebih siap.',
            ],
            needs_action: [
                'Segera lakukan absensi sesuai jadwal agar catatan kehadiran tetap aman.',
                'Masih ada waktu untuk memperbaiki kedisiplinan hari ini. Yuk, mulai sekarang.',
                'Jangan tunda absensi. Disiplin hari ini menentukan progres belajar ke depan.',
                'Ambil langkah sekarang: pastikan absensi Anda tercatat dengan benar.',
            ],
        };

        return patchedPickRandom(motivationalMessages[level]) || 'Tetap semangat dan jaga kedisiplinan belajar setiap hari.';
    }

    function patchedApplyStudentStatusTheme(status, context = {}) {
        const theme = patchedStudentStatusTheme(status);

        const badge = document.getElementById('dashStatusBadge');
        if (badge) {
            badge.className = `inline-flex items-center gap-2 px-4 py-2 rounded-xl border backdrop-blur-md text-xs font-bold ${theme.badge}`;
            badge.textContent = theme.label;
        }

        patchedSetText('studentStatusText', theme.label, 'Belum Absen');
        patchedSetText('studentStatusHint', theme.hint, '-');
        patchedSetText('studentGreetingSubtext', patchedResolveStudentMotivation(status, context) || theme.subtitle, '-');
        ...
        if (hero) {
            hero.classList.remove(
                ...
                'from-amber-900', 'via-orange-800',
                'from-rose-900', 'via-red-800',
                'from-cyan-900', 'via-sky-800', 'to-indigo-900'
            );
            hero.classList.add(...theme.hero);
        }

        const progressBar = document.getElementById('studentAttendanceProgressBar');
        if (progressBar) {
            progressBar.classList.remove('bg-emerald-300', 'bg-blue-300', 'bg-amber-300', 'bg-rose-300', 'bg-indigo-300', 'bg-cyan-300');
            progressBar.classList.add(theme.progress);
        }
    }
    ...
    window.loadDashboardData = function loadDashboardDataPatched() {
        ...
        patchedRun('getDashboardSummary', {
            kelas: kelasFilter || '',
        })
            .then((res) => {
                ...
                const siswaRow = payload.student || null;
                const attendanceMode = normalizeDashboardAttendanceMode(payload.attendanceMode || '');
                const hadir = Number(summary.hadir || 0);
                const masuk = Number(summary.masuk || 0);
                const sakit = Number(summary.sakit || 0);
                const izin = Number(summary.izin || 0);
                const alpa = Number(summary.alpa || 0);
                const belum = Number(summary.belum || 0);
                const total = Number(summary.total || 0);
                const totalAktif = Number(summary.totalAktif || 0);
                const counts = { hadir, masuk, sakit, izin, alpa, belum };
                ...
                patchedSetText('statGuruHadir', hadir);
                patchedSetText('statGuruSakit', sakit);
                patchedSetText('statGuruIzin', izin);
                patchedSetText('statGuruAlpa', alpa);
                applyOperationalDashboardState(attendanceMode, masuk);

                if (document.getElementById('view-siswa-dashboard')) {
                    const displayName = String(currentUser?.nama || currentUser?.name || currentUser?.username || 'Siswa').trim();
                    const nisn = String(currentUser?.nisn || currentUser?.username || '').trim();
                    const statusSiswa = String(siswaRow?.status || 'Belum Absen').trim() || 'Belum Absen';
                    const isNeedAction = statusSiswa === 'Belum Absen' || statusSiswa === 'Alpa' || statusSiswa === 'Alfa';
                    const attendanceRate = totalAktif > 0 ? Math.round((hadir / totalAktif) * 100) : 0;
                    const izinSakitCount = izin + sakit;
                    const needActionCount = alpa + belum;

                    patchedSetText('dashGreeting', displayName || 'Siswa');
                    patchedSetText('profileNameSidebar', displayName || '-');
                    patchedSetText('profileNisnSidebar', nisn || '-');
                    patchedSetText('profileKelasSidebar', currentUser?.kelas || siswaRow?.kelas || '-');
                    patchedSetText('valMasuk', siswaRow?.jamDatang || '--:--');
                    patchedSetText('valPulang', siswaRow?.jamPulang || '--:--');

                    patchedApplyStudentStatusTheme(statusSiswa, {
                        attendanceRate,
                        jamDatang: siswaRow?.jamDatang || '',
                    });
                    patchedSetText('studentClassTotal', totalAktif, '0');
                    patchedSetText('studentClassHadir', hadir, '0');
                    patchedSetText('studentClassIzinSakit', izinSakitCount, '0');
                    patchedSetText('studentClassNeedAction', needActionCount, '0');
                    patchedSetText('studentAttendanceRate', `${attendanceRate}%`, '0%');

                    const progressBar = document.getElementById('studentAttendanceProgressBar');
                    if (progressBar) {
                        progressBar.style.width = `${Math.min(Math.max(attendanceRate, 0), 100)}%`;
                    }

                    if (totalAktif <= 0) {
                        patchedSetText('studentAttendanceSummary', 'Hari ini tidak ada jadwal kehadiran karena kelas sedang libur.');
                    } else {
                        patchedSetText('studentAttendanceSummary', `${hadir} dari ${totalAktif} siswa hadir di kelas hari ini.`);
                    }

                    const alertBelum = document.getElementById('alertBelumAbsen');
                    if (alertBelum) {
                        if (!siswaRow || isNeedAction) alertBelum.classList.remove('hidden');
                        else alertBelum.classList.add('hidden');
                    }
                }

                if (document.getElementById('adminAttendanceChart')) {
                    safelyRenderDashboardChart(() => renderAdminChart(attendanceMode, counts));
                }
                if (document.getElementById('guruAttendanceChart')) {
                    safelyRenderDashboardChart(() => renderGuruChart(attendanceMode, counts));
                }
                if (document.getElementById('studentAttendanceChart')) {
                    safelyRenderDashboardChart(() => renderStudentChart(attendanceMode, counts));
                }
            })
            .catch((err) => showAlert('error', err.message || String(err)));
    };
