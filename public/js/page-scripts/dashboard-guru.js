...
function loadAdminDashboard() {
    ...
    function normalizeMonitoringStatusLabel(label) {
        const normalized = String(label || '').trim();
        return normalized === 'Alfa' ? 'Alpa' : normalized;
    }

    function openMonitoringDetailFromChart(label, value) {
        const status = normalizeMonitoringStatusLabel(label);
        if (!status || Number(value || 0) <= 0) {
            return;
        }

        const base = String(window.APP_ROUTES?.monitoring || '/monitoring').trim();
        if (!base) {
            return;
        }
    ...
    function applyDashboardChartSummary(prefix, mode, counts, total) {
        const safeTotal = Math.max(0, Number(total || 0));
        const recorded = Math.max(0, safeTotal - Number(counts?.belum || 0));
        const attention = Math.max(0, Number(counts?.belum || 0) + Number(counts?.alpa || 0));
        const series = buildAttendanceChartSeries(mode, counts || {});

        let dominantLabel = 'Belum ada data';
        let dominantValue = 0;

        (Array.isArray(series?.labels) ? series.labels : []).forEach((label, index) => {
            const currentValue = Number(Array.isArray(series?.values) ? series.values[index] : 0);
            if (currentValue > dominantValue) {
                dominantValue = currentValue;
                dominantLabel = String(label || 'Belum ada data');
            }
        });

        patchedSetText(`${prefix}ChartRecordedCount`, recorded, '0');
        patchedSetText(
            `${prefix}ChartRecordedHint`,
            safeTotal > 0
                ? `${recorded} dari ${safeTotal} siswa sudah memiliki status hari ini.`
                : 'Belum ada data siswa yang tampil hari ini.',
            '-'
        );
        patchedSetText(`${prefix}ChartDominantLabel`, dominantLabel, 'Belum ada data');
        patchedSetText(
            `${prefix}ChartDominantHint`,
            dominantValue > 0
                ? `${dominantValue} siswa berada pada status ${dominantLabel.toLowerCase()}.`
                : 'Status dominan akan muncul setelah data masuk.',
            '-'
        );
        patchedSetText(`${prefix}ChartAttentionCount`, attention, '0');
        patchedSetText(
            `${prefix}ChartAttentionHint`,
            attention > 0
                ? `${attention} siswa masih perlu diperiksa atau ditindaklanjuti.`
        ...
        return new Chart(context, {
            ...
            options: {
                ...
                onClick: (event, elements, chart) => {
                    if (!elements.length || typeof theme.onPointClick !== 'function') {
                        return;
                    }

                    const point = elements[0];
                    const label = Array.isArray(chart?.data?.labels) ? chart.data.labels[point.index] : '';
                    const dataset = Array.isArray(chart?.data?.datasets) ? chart.data.datasets[point.datasetIndex] : null;
                    const value = Array.isArray(dataset?.data) ? dataset.data[point.index] : 0;
                    theme.onPointClick({
                        index: point.index,
    ...
    function renderAdminChart(mode, counts) {
        ...
        adminChartInstance = createAttendanceAreaChart(canvas, 'Jumlah Siswa', buildAttendanceChartSeries(mode, counts), {
            ...
            labelColor: '#4338CA',
            onPointClick: ({ label, value }) => openMonitoringDetailFromChart(label, value)
        });
    }
    ...
    window.loadDashboardData = function loadDashboardDataPatched() {
        ...
        patchedRun('getDashboardSummary', {
            ...
            .then((res) => {
                ...
                applyDashboardChartSummary('admin', attendanceMode, counts, total);
                applyDashboardChartSummary('guru', attendanceMode, counts, total);

                if (document.getElementById('view-siswa-dashboard')) {
                    const nisn = String(currentUser?.nisn || currentUser?.username || '').trim();
                    const displayName = String(currentUser?.nama || currentUser?.username || 'Siswa').trim();
                    ...
                    patchedSetText('dashStatusBadge', statusSiswa);

                    const alertBelum = document.getElementById('alertBelumAbsen');
                    if (alertBelum) {
                        if (!siswaRow || statusSiswa === 'Belum Absen') alertBelum.classList.remove('hidden');
