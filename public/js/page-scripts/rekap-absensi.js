...
function getSingleClassScopeKelas() {
    const kelas = String(currentUser?.kelas || '').trim();
    if (!kelas) return '';

    const roleKey = String(role || currentUser?.role || '').toLowerCase();
    const unrestrictedRoles = ['super-admin', 'superadmin', 'admin', 'kepsek', 'wakasek'];
    if (unrestrictedRoles.includes(roleKey)) return '';

    return kelas;
}

function applySingleClassFilterLock(selectId = 'fKelasRekap') {
    const select = document.getElementById(selectId);
    if (!select) return false;

    const forcedKelas = getSingleClassScopeKelas();
    if (!forcedKelas) {
        const wasLocked = select.disabled;
    ...
    for (const opt of select.options) {
        if (String(opt.value).trim() === forcedKelas) {
            hasOption = true;
            break;
        }
    }
    ...
    const changed = select.value !== forcedKelas || !select.disabled;
...
function populateAllClassDropdowns() {
    ...
    const classFilterChanged = applySingleClassFilterLock('fKelasRekap');
    if (
        classFilterChanged &&
        document.getElementById('fStart') &&
        document.getElementById('fEnd') &&
...
const rekapAbsensiState = {
    selectedDay: null,
};

function getRekapAbsensiHeaderSelection() {
    const bulanEl = document.getElementById('rekapAbsensiBulan');
    const tahunEl = document.getElementById('rekapAbsensiTahun');
    const kelasEl = document.getElementById('fKelasRekap');

    const now = new Date();
    const bulan = Math.max(1, Math.min(12, parseInt(bulanEl?.value || String(now.getMonth() + 1), 10) || (now.getMonth() + 1)));
    const tahun = parseInt(tahunEl?.value || String(now.getFullYear()), 10) || now.getFullYear();
    const kelas = String(kelasEl?.value || '').trim();

    return { bulan, tahun, kelas };
}
...
function syncRekapAbsensiDateInputs() {
    const start = document.getElementById('fStart');
    const end = document.getElementById('fEnd');
    if (!start || !end) return;

    const { bulan, tahun } = getRekapAbsensiHeaderSelection();
    const maxDay = new Date(tahun, bulan, 0).getDate();
    let day = parseInt(rekapAbsensiState.selectedDay || 0, 10);
    if (!Number.isFinite(day) || day < 1) day = 1;
    if (day > maxDay) day = maxDay;
    rekapAbsensiState.selectedDay = day;

    const iso = toIsoDate(tahun, bulan, day);
    start.value = iso;
    end.value = iso;
}

function setRekapAbsensiDayTabActive(dayValue) {
    const tabs = document.querySelectorAll('#rekapAbsensiDayTabs .rekap-day-tab');
    const normalized = String(dayValue || '');
    tabs.forEach((tab) => {
        const isActive = tab.dataset.day === normalized;
...
function renderRekapAbsensiDayTabs() {
    const container = document.getElementById('rekapAbsensiDayTabs');
    if (!container) return;
    ...
    const now = new Date();
    const isCurrentMonth = now.getFullYear() === tahun && (now.getMonth() + 1) === bulan;
    let selectedDay = parseInt(rekapAbsensiState.selectedDay || 0, 10);
    if (!Number.isFinite(selectedDay) || selectedDay < 1) {
        selectedDay = isCurrentMonth ? now.getDate() : 1;
    }
    if (selectedDay > maxDay) selectedDay = maxDay;
    rekapAbsensiState.selectedDay = selectedDay;

    const tabHtml = [];
    for (let d = 1; d <= maxDay; d++) {
        tabHtml.push(
            `<button type="button" class="rekap-day-tab px-3 py-1.5 rounded-lg text-xs font-bold border border-gray-200 bg-white text-gray-600" data-day="${d}" onclick="selectRekapAbsensiDay(${d})">${d}</button>`
        );
    }
    container.innerHTML = tabHtml.join('');
    setRekapAbsensiDayTabActive(selectedDay);
}

function selectRekapAbsensiDay(day) {
    const selected = parseInt(day, 10);
    if (!Number.isFinite(selected) || selected < 1) return;
    rekapAbsensiState.selectedDay = selected;
    setRekapAbsensiDayTabActive(selected);
    applyFilter();
}

window.selectRekapAbsensiDay = selectRekapAbsensiDay;

function handleRekapAbsensiHeaderFilterChange() {
    renderRekapAbsensiDayTabs();
    applyFilter();
}

window.handleRekapAbsensiHeaderFilterChange = handleRekapAbsensiHeaderFilterChange;

function renderRekapLoadingState() {
...
function applyFilter() {
        ...
        actionRunner.withSuccessHandler(result => {
            ...
            const tbody = document.getElementById('tbody-rekap');
            if (tbody) {
                tbody.innerHTML = `<tr><td colspan="8" class="p-8 text-center text-red-500">${String(err || 'Gagal memuat data rekap.')}</td></tr>`;
            }
...
function handleArchiveAndReset() {
    ...
    function patchedInitRekapYear(selectId = 'rekapTahun') {
        const select = document.getElementById(selectId);
    ...
    function patchedInitRekapAbsensiHeaderDefaults() {
        const monthSelect = document.getElementById('rekapAbsensiBulan');
        const yearSelect = document.getElementById('rekapAbsensiTahun');
        if (!monthSelect || !yearSelect) return;

        patchedInitRekapYear('rekapAbsensiTahun');
        applySingleClassFilterLock('fKelasRekap');

        const now = new Date();
        monthSelect.value = String(now.getMonth() + 1);
        yearSelect.value = String(now.getFullYear());
        rekapAbsensiState.selectedDay = now.getDate();
    }
    ...
    function bootstrapCurrentPage() {
        ...
        if (
            document.getElementById('rekapAbsensiBulan') &&
            document.getElementById('rekapAbsensiTahun') &&
            document.getElementById('fStart') &&
            document.getElementById('fEnd') &&
            typeof applyFilter === 'function'
        ) {
            patchedInitRekapAbsensiHeaderDefaults();
            renderRekapAbsensiDayTabs();
            applyFilter();
        } else if (document.getElementById('fStart') && document.getElementById('fEnd') && typeof applyFilter === 'function') {
